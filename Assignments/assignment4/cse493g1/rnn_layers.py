...
def rnn_step_backward(dnext_h, cache):
    ...
    tanh_grad = 1 - next_h**2
    dtemp = dnext_h * tanh_grad
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
      dhtemp = dh[:,i,:] + dprev_h
      dx_t, dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dhtemp, cache[i])
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    dnext_c_temp = dnext_h * o * ( 1 - np.tanh(next_c)**2 )
    dnext_c_tot = dnext_c + dnext_c_temp
...
def lstm_backward(dh, cache):
    ...
    for i in reversed(range(T)):
      ...
      dx_t, dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h, dprev_c, cache[i])
