...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        num_k_N = torch.exp(sim(z_k_N, z_k)/ tau)

        den_k = torch.sum(torch.tensor([torch.exp(sim(z_k, out[j]) / tau) for j in range(2 * N) if j != k]))
        den_k_N = torch.sum(torch.tensor([torch.exp(sim(z_k_N, out[j]) / tau) for j in range(2 * N) if j != k + N]))

        loss_k = -torch.log(num_k/ den_k)
        loss_k_N = -torch.log(num_k_N / den_k_N)
